...
PropSSBbrSender::DebugState::DebugState(const PropSSBbrSender& sender)
    : mode(sender.mode_),
...
PropSSBbrSender::DebugState::DebugState(const DebugState& state) = default;

  PropSSBbrSender::PropSSBbrSender(const QuicClock* clock, const RttStats* rtt_stats,
...
PropSSBbrSender::~PropSSBbrSender() {}

void PropSSBbrSender::SetAuxiliaryClientData(ClientData* cdata) {
    client_data_ = cdata;
}

bool PropSSBbrSender::InSlowStart() const {
  return mode_ == STARTUP;
}

void PropSSBbrSender::OnPacketSent(QuicTime sent_time,
...
bool PropSSBbrSender::CanSend(QuicByteCount bytes_in_flight) {
  return bytes_in_flight < GetCongestionWindow();
}

QuicBandwidth PropSSBbrSender::PacingRate(QuicByteCount bytes_in_flight) const {
...
QuicBandwidth PropSSBbrSender::BandwidthEstimate() const {
  return max_bandwidth_.GetBest();
}

QuicByteCount PropSSBbrSender::GetCongestionWindow() const {
  if (mode_ == PROBE_RTT) {
...
QuicByteCount PropSSBbrSender::GetSlowStartThreshold() const {
  return 0;
}

float PropSSBbrSender::PacingGainAtOffset(int offset) const {
    if (offset > 0) {
        return kPacingGain[offset];
    }
    float gain = kPacingGain[0];
    if (client_data_ != nullptr) {
        float ss = client_data_->get_screen_size();
        gain = 1 + ss*(gain-1)/2;
    }
    return gain;
}

bool PropSSBbrSender::InRecovery() const {
  return recovery_state_ != NOT_IN_RECOVERY;
}

bool PropSSBbrSender::IsProbingForMoreBandwidth() const {
  return (mode_ == PROBE_BW && pacing_gain_ > 1) || mode_ == STARTUP;
}

void PropSSBbrSender::SetFromConfig(const QuicConfig& config,
...
void PropSSBbrSender::AdjustNetworkParameters(QuicBandwidth bandwidth,
                                        QuicTime::Delta rtt) {
...
void PropSSBbrSender::OnCongestionEvent(bool /*rtt_updated*/,
...
CongestionControlType PropSSBbrSender::GetCongestionControlType() const {
  return kBBR;
}

QuicTime::Delta PropSSBbrSender::GetMinRtt() const {
  return !min_rtt_.IsZero()
             ? min_rtt_
             : QuicTime::Delta::FromMicroseconds(rtt_stats_->initial_rtt_us());
}

QuicByteCount PropSSBbrSender::GetTargetCongestionWindow(float gain) const {
...
QuicByteCount PropSSBbrSender::ProbeRttCongestionWindow() const {
...
void PropSSBbrSender::EnterStartupMode() {
  mode_ = STARTUP;
  pacing_gain_ = kHighGain;
  congestion_window_gain_ = kHighGain;
}

void PropSSBbrSender::EnterProbeBandwidthMode(QuicTime now) {
  mode_ = PROBE_BW;
  ...
  pacing_gain_ = PacingGainAtOffset(cycle_current_offset_);
}

void PropSSBbrSender::DiscardLostPackets(const LostPacketVector& lost_packets) {
  for (const LostPacket& packet : lost_packets) {
    sampler_->OnPacketLost(packet.packet_number);
  }
}

bool PropSSBbrSender::UpdateRoundTripCounter(QuicPacketNumber last_acked_packet) {
...
bool PropSSBbrSender::UpdateBandwidthAndMinRtt(
    QuicTime now,
    const AckedPacketVector& acked_packets) {
...
bool PropSSBbrSender::ShouldExtendMinRttExpiry() const {
...
void PropSSBbrSender::UpdateGainCyclePhase(QuicTime now,
                                     QuicByteCount prior_in_flight,
                                     bool has_losses) {
...
void PropSSBbrSender::CheckIfFullBandwidthReached() {
...
void PropSSBbrSender::MaybeExitStartupOrDrain(QuicTime now) {
...
void PropSSBbrSender::MaybeEnterOrExitProbeRtt(QuicTime now,
                                         bool is_round_start,
                                         bool min_rtt_expired) {
...
void PropSSBbrSender::UpdateRecoveryState(QuicPacketNumber last_acked_packet,
                                    bool has_losses,
                                    bool is_round_start) {
...
void PropSSBbrSender::UpdateAckAggregationBytes(QuicTime ack_time,
...
void PropSSBbrSender::CalculatePacingRate() {
...
void PropSSBbrSender::CalculateCongestionWindow(QuicByteCount bytes_acked) {
  if (mode_ == PROBE_RTT) {
    return;
  }
...
void PropSSBbrSender::CalculateRecoveryWindow(QuicByteCount bytes_acked,
...
std::string PropSSBbrSender::GetDebugState() const {
  std::ostringstream stream;
  stream << ExportDebugState();
  return stream.str();
}

void PropSSBbrSender::OnApplicationLimited(QuicByteCount bytes_in_flight) {
...
PropSSBbrSender::DebugState PropSSBbrSender::ExportDebugState() const {
  return DebugState(*this);
}

static std::string ModeToString(PropSSBbrSender::Mode mode) {
  switch (mode) {
    case PropSSBbrSender::STARTUP:
      return "STARTUP";
    case PropSSBbrSender::DRAIN:
      return "DRAIN";
    case PropSSBbrSender::PROBE_BW:
      return "PROBE_BW";
    case PropSSBbrSender::PROBE_RTT:
      return "PROBE_RTT";
  }
  return "???";
}

std::ostream& operator<<(std::ostream& os, const PropSSBbrSender::Mode& mode) {
  os << ModeToString(mode);
  return os;
}

std::ostream& operator<<(std::ostream& os, const PropSSBbrSender::DebugState& state) {
  ...
  if (state.mode == PropSSBbrSender::STARTUP) {
